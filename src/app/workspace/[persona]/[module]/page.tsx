...
> = {
  "patient:booking": {
    ...
    summary: "Choose provider, confirm slot, and create a billable visit in one flow.",
    tips: [
      "Pick the provider first, then exact schedule to avoid validation errors.",
      "Use consultation fee input to test billing scenarios with different amounts.",
      "After booking, open Visits module to continue check-in and payment flow.",
    ],
  },
  "patient:visits": {
    title: "Patient Visits Flow",
    summary: "Track consultation state, pay invoice, and monitor prescriptions/care orders.",
    tips: [
      "Use Check In before consultation starts to move session readiness.",
      "Use Pay Now with payment method to complete financial closure.",
      "Watch status chips to verify appointment and consultation sync.",
    ],
  },
  "patient:inbox": {
    title: "Patient Communication Flow",
    summary: "Review reminders, claim updates, and communication timeline.",
    tips: [
      "Unread notifications can be acknowledged from this screen.",
      "Destination and delivery detail helps validate channel behavior.",
  ...
  "provider:dashboard": {
    ...
    tips: [
      "Keep consultation status updated to maintain queue accuracy.",
      "Submit claims only when invoice data is available for the appointment.",
      "Use queued dispatch button for scheduled or deferred notifications.",
    ],
  },
  "provider:schedule": {
    title: "Provider Capacity Flow",
    summary: "Create and maintain availability blocks for consultation booking.",
    tips: [
      "Timezone and slot duration affect appointment visibility.",
      "Keep inactive slots disabled instead of deleting for audit traceability.",
      "Use weekdays in sequence to maintain predictable patient booking behavior.",
    ],
  },
  "provider:patients": {
    title: "Provider Clinical Flow",
    summary: "Complete notes, prescriptions, and care orders with medical traceability.",
    tips: [
      "Save SOAP notes first, then issue prescription and care orders.",
      "Sign note only after final review to lock finalized documentation.",
  ...
  "admin:pulse": {
    ...
    tips: [
      "Review appointment mix daily to identify throughput bottlenecks.",
      "Track paid vs pending invoice trend for financial health.",
      "Use queue snapshot to validate provider-side execution quality.",
    ],
  },
  "admin:operations": {
    title: "Admin Control Flow",
    summary: "Operate appointment states, claims, incidents, compliance, and permissions.",
    tips: [
      "Use status dropdowns instead of quick chips for controlled updates.",
      "Enter review or resolution notes whenever status changes.",
      "Use dispatch/retry controls to clear communication backlog.",
    ],
  },
  "admin:audit": {
    title: "Audit Governance Flow",
    summary: "Review immutable activity trail across clinical and operations actions.",
    tips: [
...
export default async function ModuleWorkspacePage({
  ...
  const moduleGuidance = MODULE_GUIDANCE[`${persona}:${module}`] ?? {
    title: moduleConfig?.title ?? "Workflow",
    summary: moduleConfig?.description ?? "Operational workflow guidance.",
    tips: ["Follow top-to-bottom panel order to complete this workflow."],
  };
  ...
  return (
    <main className="prototype-bg workspace-main min-h-[calc(100vh-60px)] px-3 py-5 sm:px-6 sm:py-8">
      <div className="mx-auto w-full max-w-[1600px] space-y-5 animate-rise">
        <section className="rounded-3xl border border-white/80 bg-white/88 p-5 shadow-[0_26px_74px_-52px_rgba(2,8,20,0.9)] backdrop-blur sm:p-7">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="rounded-2xl border border-white bg-white/95 px-4 py-3 text-right shadow-[0_16px_40px_-32px_rgba(8,145,178,0.85)]">
              <p className="text-xs font-semibold tracking-[0.12em] text-[color:var(--role-primary)]">
                ACTIVE USER
              </p>
        ...
        <section className="rounded-2xl border border-white/80 bg-white/90 p-4 shadow-[0_16px_45px_-35px_rgba(15,23,42,0.75)]">
          <p className="text-xs font-semibold tracking-[0.14em] text-slate-500">SCREEN GUIDE</p>
          <h2 className="mt-1 text-lg font-semibold text-slate-900">{moduleGuidance.title}</h2>
          <p className="mt-1 text-sm text-slate-600">{moduleGuidance.summary}</p>
          <div className="mt-3 grid gap-2 md:grid-cols-3">
            {moduleGuidance.tips.map((tip) => (
              <p key={tip} className="rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-2 text-xs text-slate-700">
                {tip}
              </p>
            ))}
          </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-3 grid gap-2 lg:grid-cols-2">
                          <form action={updateConsultationStatusAction} className="rounded-lg border border-slate-200 bg-slate-50/70 p-2.5">
                            <input type="hidden" name="return_to" value={currentPath} />
                            <input type="hidden" name="appointment_id" value={item.id} />
                            ...
                            <div className="mt-1.5 flex gap-2">
                              <select
                                ...
                                title="Update live consultation state for this appointment"
                              ...
                              <button
                                type="submit"
                                title="Save consultation status"
                                className="rounded-md border border-slate-300 bg-white px-2.5 py-1 text-xs font-semibold text-slate-700 hover:border-slate-400"
                              >
                                Save
                              </button>
                            ...
                            <p className="mt-1 text-[11px] text-slate-500">
                              Keep this aligned with provider progress and patient readiness.
                            </p>
                          </form>

                          <form action={updateAppointmentStatusAction} className="rounded-lg border border-slate-200 bg-slate-50/70 p-2.5">
                            <input type="hidden" name="return_to" value={currentPath} />
                            <input type="hidden" name="appointment_id" value={item.id} />
                            <label className="block text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-600">
                              Appointment Status
                            ...
                            <div className="mt-1.5 flex gap-2">
                              <select
                                ...
                                title="Update business status for billing and reporting"
                                className="flex-1 rounded-md border border-slate-300 bg-white px-2 py-1 text-xs text-slate-800"
                              >
                            ...
                            <p className="mt-1 text-[11px] text-slate-500">
                              This drives claims, invoices, and dashboard KPI calculations.
                            </p>
                          </form>
                        </div>
                        {appointmentInvoices.length > 0 ? (
